...
class CustomPDF(FPDF):
    ...
    def header(self):
        ...
        header_text = f"Name: {first_name}   User ID: {self.chat_id}   PDF ID: {generate_pdf_id()}"
    ...
    def footer(self):
        ...
        footer_text = "Note: Don't share your PDF ID as it is useful to check your details and send you data."
...
def generate_pdf(message, cookies, balance_info):
    ...
    # Add Name, User ID, and PDF ID
    first_name = user_data.get(chat_id, {}).get('first_name', 'Unknown')
    pdf_id = generate_pdf_id()
    pdf.cell(0, 10, txt=f"Name: {first_name}\nUser ID: {chat_id}\nPDF ID: {pdf_id}", ln=True, align="C")
    pdf.ln(10)

    # Add cookies info (masked for security)
    cookies_text = f"Cookies: {cookies[:8]}******{cookies[-8:]}"
    pdf.cell(0, 10, txt=pdf._sanitize_text(cookies_text), ln=True, align="C")
    pdf.ln(10)

    # Add balance information in the table
